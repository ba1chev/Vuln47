...
class DataPreprocessingPipeline(PreprocessingPipeline[str, list[Data]]):
    """Turns a raw JSONL split into a list of model-ready PyG 'Data' graphs.

    Chains the four preprocessing stages — fetch (if missing) -> load records ->
    parse each function into an AST 'CodeGraph' -> featurize nodes and assemble
    a PyG 'Data' object. 'run' returns the graphs in memory; the notebooks
    persist them to 'data/processed/*.pt'.
    """
    ...
    def run(self, input: str) -> list[Data]:
        # self-heal: download the split if it is not present yet
        ...
        for rec in tqdm(self.loader.load(input), desc="preprocess"):
            ...
            if code_graph is None:  # unparsable / too large / too small — drop it
                continue
    ...
    def _to_data(self, graph: CodeGraph, label: int) -> Data:
        """Assemble one 'CodeGraph' + label into a PyG 'Data' object."""
        # x: [num_nodes, 7] float feature matrix, one row per node
        feats = [
        ...
        if graph.edges:
            ...
            # make every edge bidirectional so messages flow up and down the AST;
            # the reverse copy gets a distinct type id (base + NUM_BASE_EDGE_TYPES)
            # so the model can still tell a downward edge from an upward one
            ...
            # a graph with no edges still needs correctly-shaped empty tensors
